...
1. **Simple API Calls** - Standard request/response pattern
2. **Streaming Responses** - Server-Sent Events (SSE) for real-time updates
3. **Request Cancellation** - Abort in-flight requests with Ctrl+C
4. **Error Handling** - Graceful error display and recovery
5. **History Management** - Send conversation context with each request

### Running the API Example

First, set your API configuration:

```bash
export API_ENDPOINT="https://your-api.com/chat"
